...
fft_size = 2048
# distance to slide along the window (in time)
step_size = fft_size // 16
# threshold for spectrograms (lower filters out more noise)
spec_thresh = 4
# Hz # Low cut for our butter bandpass filter
lowcut = 500
# Hz # High cut for our butter bandpass filter
highcut = 15000

# number of mel frequency channels
n_mel_freq_components = 64
# how much should we compress the x-axis (time)
shorten_factor = 10
# What frequency to start sampling our melS from / Hz
start_freq = 300
# What frequency to stop sampling our melS from / Hz
end_freq = 8000


# Most of the Spectrograms and Inversion are taken from: https://gist.github.com/kastnerkyle/179d6e9a88202ab0a2fe
# thanks also to https://timsainburg.com/python-mel-compression-inversion.html
...
def overlap(X, window_size, window_step):
    """ Create an overlapped version of X
        ...
        Returns: X_strided : shape=(n_windows, window_size) : 2D array of overlapped X"""
    if window_size % 2 != 0:
...
def createMelFromAudio(audio_path, output_dir):
    rate, data = wavfile.read(audio_path)
    data = butter_bandpass_filter(data, lowcut, highcut, rate, order=1)
    # length is time is np.shape(data)[0] / float(rate)
    ...
    mel_filter, mel_inversion_filter = create_mel_filter(
        ...
        end_freq=end_freq)
    # create the image. Now we need to save it
    mel_spec = make_mel(wav_spectrogram, mel_filter, shorten_factor=shorten_factor)
    # after this we need to save the image


def createAudioFromMel(image_path, output_dir):
    # we need to load the image here
    image = image_path
    mel_filter, mel_inversion_filter = create_mel_filter(
        fft_size=fft_size,
        n_freq_components=n_mel_freq_components,
    ...
    inverted_mel_audio = invert_pretty_spectrogram(
        ...
        log=True,
        n_iter=10)
    # this is the inverted audio, now we must save it
    print(inverted_mel_audio)


if __name__ == '__main__':
    creatMelFromAudio('../tests/resources/WAV')
