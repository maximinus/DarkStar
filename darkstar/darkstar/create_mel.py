...
# code taken and adapted from https://timsainburg.com/python-mel-compression-inversion.html
...
def overlap(X, window_size, window_step):
    ...
    # There is no point adding zero data, better to reduce the data so it fits
    data_to_remove = len(X) % window_size
    X = X[:-data_to_remove]
    ...
    print(nw, window_size)

    print('----')


    # for all nw (1712 here)
    for i in np.arange(nw):
        # "slide" the window along the samples
        # i.e. move along 0, then the step size etc etc
        start = i * window_step
        stop = start + window_size
        #print(start, stop)
        # then copy this along the y axis
        out[i] = X[start:stop]
...
def stft(X, fftsize=128, step=65, mean_normalize=True, real=False, compute_onesided=True):
    ...
    # this rakes a shape of (441,000) and converts to (1720, 1024)
    # we want to control this shape
...
if __name__ == '__main__':
    ...
    mywav = getDataDirectory('TMP') + '/input_mono.wav'
    ...
    save_path = getDataDirectory('TMP') + '/large_mel.png'
    plt.savefig(save_path, bbox_inches=None, pad_inches=0, dpi=80)
